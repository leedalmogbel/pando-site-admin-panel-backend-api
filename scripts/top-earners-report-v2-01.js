...
function generateReport() {
	fs.writeFile(`top-earners-report/top-earners-report-v2-01.txt`, '', function() {
		const writer = fs.createWriteStream(`top-earners-report/top-earners-report-v2-01.txt`, {
										    			...
										    			if(userPlan) {
										    				Plans.findById(userPlan.purchased_plan_id)
										    					.then(plan => {
										    						...
										    						writer.write(`${userDetails.email}%${userDetails.date_registered}%${userDetails.elapsed_time}%${userDetails.elapsed_pando_earned}%${userDetails.referral_pando_earned}%${userDetails.passive_mining_elapsed_pando_earned}%${userDetails.total_earnings}%${userDetails.total_withdraw}%${planAmount}\n`);
										    					})
										    					.catch(error => {
										    						console.log(error);
										    					});
